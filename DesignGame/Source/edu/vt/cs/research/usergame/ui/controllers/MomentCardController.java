...
public class MomentCardController implements MouseListener, MouseMoveListener
{
    ...
    {
    	momentCard.temp =1;
        game.getStage().getDragCard().setVisible(false);

    	mouseDownTemp=new Point(e.x,e.y);
        try
        {
    ...
    private boolean possess() throws TupleSpaceException
    {
        ...
        try
        {
            ...
            if (sharedTuple.getSpace().equals(MomentCardTuple.STAGE_SPACE))
            {
                ...
                //momentCard.setLocation(sharedTuple.getX(), sharedTuple.getY());
                //momentCard.localX=20;
                //momentCard.localY=20;
                momentCard.setStageLocation(sharedTuple.getX(), sharedTuple.getY());
                ...
                dragCard.setLocation(sharedTuple.getX(), sharedTuple.getY());
                //dragCard.setStageLocation(sharedTuple.getX(), sharedTuple.getY());
                ...
                if (mbDown[e.button])
                {
                    ...
                    if (e.button == LMB)
                    {
                        ...
                        // This card was originally on the board
                        {
                            ...
                            {
                            	System.out.println(""+dragSpace.getDragCard().getLocation());
                                momentCard.getOtherCard().localX=dragSpace.getDragCard().getLocation().x;
                                momentCard.getOtherCard().localY=dragSpace.getDragCard().getLocation().y;
                        ...
                        // This card was originally on the stage
                        {
                            ...
                            else if (dragSpace == game.getStage())//e.x/e.y gives me the offset of where you click on the picture.
                            // This card is still on the stage    //momentCard.toDisplay(new Point(e.x, e.y)); give me absolute ased on top left corner
                            {
                            	momentCard.localX+=e.x-mouseDownTemp.x;//TODO
                            	momentCard.localY+=e.y-mouseDownTemp.y;//TODO
                            	placeStageMomentOnStage(e.x,e.y);
                            }
                        }
    ...
    private void placeBoardMomentOnStage() throws TupleSpaceException
    {
        ...
        try
        {
            ...
            //Stuff probably needs to go here
    ...
    }//momentCard.toDisplay(new Point(e.x,e.y))
    private void placeStageMomentOnStage(int ex,int ey) throws TupleSpaceException
    {
        game.getTupleSpace().waitToTake(ControlTuple.BOARD_CONTROL_TUPLE);
        try
        {
            ...
            MomentCard dragCard = dragSpace.getDragCard();
            //dragCard.localX =momentCard.toDisplay(new Point(ex,ey)).x;
            //dragCard.localY = momentCard.toDisplay(new Point(ex,ey)).y;
            //System.out.println("LocalX ="+dragCard.localX+"  LocalY ="+dragCard.localY);
        	//System.out.println("***"+ex+"  "+ey);
        	//System.out.println("FinalValue="+(dragCard.localX-(game.getStage().toDisplay(new Point(0,0)).x)-mouseDownTemp.x)+"  "+(dragCard.localY-(game.getStage().toDisplay(new Point(0,0)).y)-mouseDownTemp.y));
            //System.out.println("***"+game.getStage().toControl());
            /*game.getStage().getMomentCard(dragCard.getSourceURL()).getTuple().setX(dragCard.getTuple().getX());
            game.getStage().getMomentCard(dragCard.getSourceURL()).getTuple().setY(dragCard.getTuple().getY());
            game.getStage().cardMoved(game.getStage().getMomentCard(dragCard.getSourceURL()));*/
            game.getTupleSpace().write(dragCard.getTuple());
        }
        catch (TupleSpaceException e)
        {
    ...
    public void cardDraggedOnStage(Point absoluteCursorLocation) throws TupleSpaceException
    {
       ...
       // momentCard.localX=newLocationOnStage.x;
       // momentCard.localY=newLocationOnStage.y;
//        ScrolledComposite stageScrolledComposite = (ScrolledComposite)game.getStage().getParent();
...
//        }
        ...
        dragCard.setLocation(newLocationOnStage);//TODO THESE TWO LINES SHOULD BE WRONG
        //dragCard.setStageLocation(newLocationOnStage);//BUT IT SEEMS TO WORK
...
//        }
        ...
        dragCard.setLocation(newLocationOnBoard);
